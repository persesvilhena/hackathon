...
class Painel extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->model('nucleo_model');
        //var_dump($this->permissao_model->verifica_permissao("lis_sua_na", 1, 1));
        $dados['min'] = $this->painel_model->retorna_palavras_por_datas($this->session->userdata['id']);
        $dados['acesso_dia'] = $this->painel_model->qtde_acessos_empresa($this->session->userdata['id']);
        $dados['palavras_chave'] = $this->painel_model->principais_palavras($this->session->userdata['id']);
        $dados['qtde_geral'] = $this->painel_model->qtde_buscas();
        $dados['qtde_suas'] = $this->painel_model->qtde_suas($this->session->userdata['id']);
    ...
    public function msg() {
        if($this->input->post('postar') != null){
            if(!empty($this->input->post('msg'))){
                $id = $this->input->post('id');
                $msg = $this->input->post('msg');
                if($this->painel_model->responde_post($id, $msg)){
                    $avisos[0]['message'] = "Postagem realizada com sucesso!";
                    $avisos[0]['type'] = "success";
                }else{
                    $avisos[1]['message'] = "Houve um problema!";
                    $avisos[1]['type'] = "danger";
                }
            }else{
                $avisos[2]['message'] = "Por favor! Preencha todos os campos!";
                $avisos[2]['type'] = "warning";
            }
            $this->session->set_userdata('avisos', $avisos);
        }

        $dados['postagens'] = $this->ver_model->postagens($this->session->userdata['id']);
        $dados['user'] = $this->painel_model->retorna_dados_usuario($this->session->userdata['id']);

        $this->load->view('header');
        $this->load->view('barra_superior', $dados);
        $this->load->view('adm/msg', $dados);
        $this->load->view('footer');
    }

    public function teste123(){
        echo "<pre>";
        var_dump($this->painel_model->qtde_acessos_empresa(1));
        echo "</pre>";
    }
    ...
    public function est_gerais() {
        ...
        $dados['acesso_dia'] = $this->painel_model->qtde_acessos();
        $dados['palavras_chave'] = $this->painel_model->principais_palavras_geral();


        $this->load->view('header');
        $this->load->view('barra_superior', $dados);
        $this->load->view('painel_geral', $dados);
    ...
    public function produtos($pag = null, $par = null) {


        $dados['user'] = $this->painel_model->retorna_dados_usuario($this->session->userdata['id']);
        $dados['produtos'] = $this->painel_model->retorna_produtos($this->session->userdata['id']);

        $this->load->view('header');
        $this->load->view('barra_superior', $dados);
        if($pag == null || $pag == "home"){
            $this->load->view('produtos/home', $dados);
        }
        ...
        if($pag == "inserir"){
                ...
                if($this->input->post('inserir') != null){
                    ...
                    if(empty($nome) || empty($descricao) || empty($preco)){
                        ...
                        $resultado = $this->painel_model->inserir($this->session->userdata['id'], $nome, $descricao, $preco);
                        if($resultado){
                            $avisos[0]['message'] = "Produto inserido com sucesso!";
                            $avisos[0]['type'] = "success";
                        }else{
               ...
               // $dados['noticia'] = $this->noticias_model->ver_noticia($par);
                $this->load->view('produtos/inserir', $dados);
        ...
        if($pag == "alterar"){
                ...
                if($this->input->post('alterar') != null){
                    ...
                    if(empty($nome) || empty($descricao) || empty($preco)){
                        ...
                        $resultado = $this->painel_model->alterar($par, $nome, $descricao, $preco);
                        if($resultado){
                            $avisos[0]['message'] = "Produto inserido com sucesso!";
                            $avisos[0]['type'] = "success";
                        }else{
                ...
                $dados['produto'] = $this->painel_model->retorna_produto($par);

                ////////////exibe o formulario para alteracao
               // $dados['noticia'] = $this->noticias_model->ver_noticia($par);
                $this->load->view('produtos/alterar', $dados);
        ...
        if($pag == "apagar"){
                        ...
                        $resultado = $this->painel_model->apagar($par);
                        if($resultado){
                            $avisos[0]['message'] = "Produto apagado com sucesso!";
                            $avisos[0]['type'] = "success";
                        }else{
                            $avisos[1]['message'] = "Houve um problema! Contate o administrador!";
                            $avisos[1]['type'] = "danger";
