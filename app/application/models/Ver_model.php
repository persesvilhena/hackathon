...
class Ver_model extends CI_Model {
    ...
    function retorna_dados_empresa($id){
        $this->db->select('*');
    ...
    function retorna_nome_atividade($code){
        $this->db->select('*');
        $this->db->from('empresas_atividades');
        $this->db->where('empresas_atividades.code', $code);
        return $this->db->get()->result()[0]->nome;
    }

    function retorna_todos_produtos($empresa){
        $this->db->select('*');
    ...
    function grava_post($empresa, $msg){
        $data = array(
        'empresa' => $empresa,
        'post' => $msg
        );
    ...
    function registra_busca($empresa, $log){
        $data = array(
        'empresa' => $empresa,
        'log' => $log
        );
        $this->db->insert('busca_empresa', $data);
