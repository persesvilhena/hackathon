<?php
$conecta = mysqli_connect("localhost", "root", "", "empresas");
...
function verifica_atividade($atividade){
	global $conecta;
	$code = limpa_codigo($atividade->code);
	$nome = $atividade->text;

	$sql = mysqli_query($conecta, "select * from atividades where code = '$code'");
	$rsql = mysqli_num_rows($sql);
	if($rsql > 0){
		return $code;
	}else{
		//echo "INSERT INTO `atividades`(`code`, `nome`) VALUES ('$code','$nome')";
		mysqli_query($conecta, "INSERT INTO `atividades`(`code`, `nome`) VALUES ('$code','$nome')");
		return $code;
	}
}

function atividades($atividade, $id){
	global $conecta;
	foreach ($atividade as $n) {

		$code = verifica_atividade($n);
		$sql = mysqli_query($conecta, "select * from empresa_atividades where codigo_empresa = '$id' and code = '$code'");
		$rsql = mysqli_num_rows($sql);
		if($rsql == 0){
			mysqli_query($conecta, "INSERT INTO `empresa_atividades`(`codigo_empresa`, `code`) VALUES ($id,$code)");
		}
...
function tenta($documento){
	$url="http://receitaws.com.br/v1/cnpj/" . limpa_codigo($documento);
